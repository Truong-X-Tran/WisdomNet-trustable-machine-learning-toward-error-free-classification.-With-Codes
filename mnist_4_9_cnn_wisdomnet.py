'''Train WisdomNet on MNIST with a CNN base network
...
y_test_cnn_pred = model.predict(X_test)
y_test_cnn_pred = np.argmax(y_test_cnn_pred, axis = 1)
y_test_actures =  np.argmax(y_test,axis = 1)


#Code for the WisdomNet start from here
...
#Create the new weights matrix and biases of the WisdomNet output layer
...
#Create the WisdomNet by replacing the output layer of the base network
#With the new prediction layer

model2 = Model(input = model.input, outputs = new_layer)

#Checking
model2.summary() # Summary of the WisdomNet model structure

model2.compile(loss=keras.losses.categorical_crossentropy,
              optimizer=keras.optimizers.Adadelta(lr=0.9),
              metrics=['accuracy'])

#Train the WisdomNet on the undecided set of data 
...
for i in range(len(y_test_pred)):
  if y_test_pred[i] == 2:
    ...
    test_c = test_c + 1
    if y_test_pred[i] == 0:
      test_pred_4 = test_pred_4 + 1
    if y_test_pred[i] == 1:
      test_pred_9 = test_pred_9 + 1
  elif y_test_pred[i] != np.argmax(y_test[i]):
    test_m = test_m + 1
...
print('WisdomNet Confusion')
print('predicted 4:', test_pred_4)
print('predicted 9:', test_pred_9)

print('Acture 4:', test_4)
print('Acture 9:', test_9)


print('CNN Net Confusion')
print(confusion_matrix(y_test_actures, y_test_cnn_pred))